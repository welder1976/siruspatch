...
--[[
	...
	for i=1, MAX_BATTLEFIELD_QUEUES do
		status, mapName, instanceID, levelRangeMin, levelRangeMax, teamSize, registeredMatch, isSoloq = GetBattlefieldStatus(i);
--[[
		if ( mapName ) then
			...
			if ( teamSize ~= 0 ) then
				if ( registeredMatch ) then
					if isSoloq then
						mapName = ARENA_RATED_MATCH.. BATTLEFIELDFRAME_SOLO_LABEL ..format(PVP_TEAMSIZE, teamSize, teamSize);
					else
...
function MiniMapBattlefieldFrame_isArena()
	MiniMapBattlefieldFrame:UpdateArt()
end
	...
	for i=1, MAX_BATTLEFIELD_QUEUES do
		...
		if ( status == "queued" or status == "confirm" ) then
			...
			if ( teamSize ~= 0 ) then
				if ( registeredMatch ) then
					if isSoloq then
						info.text = ARENA_RATED_MATCH.. BATTLEFIELDFRAME_SOLO_LABEL ..format(PVP_TEAMSIZE, teamSize, teamSize);
					else
			...
			info.func = function (self, ...)
				if IsActiveBattlefieldArena() then
					...
					if not GetBattlefieldWinner() then
						SendServerMessage("ACMSG_BG_DESERTION_REQUEST")
					else
						LeaveBattlefield()
					end
				end
