...
function ezSpectator_GossipWorker:Create(Parent)
    ...
    self.MainFrame.Line2:SetText(ARENA_SPECTATOR_MATCH_REPLAY)
...
function ezSpectator_GossipWorker:ParseMatch(Data, Index)
    ...
    for Parameter in Data:gmatch("[^|]+") do
        ...
        if string.find(Parameter, ARENA_SPECTATOR_START_MATCH_PATTERN) then
            Parameter = string.gsub(Parameter, ARENA_SPECTATOR_START_MATCH_PATTERN, '')
        end
...
function ezSpectator_GossipWorker:ShowMatch(Match)
    ...
    if not Match.MainFrame then
        ...
        Match.MainFrame.CentralFrame.Info1:SetFont('Interface\\CustomFonts\\RobotoCondensed-Regular.ttf', 14, 'OUTLINE')
        ...
        Match.MainFrame.CentralFrame.Info2:SetFont('Interface\\CustomFonts\\RobotoCondensed-Regular.ttf', 14, 'OUTLINE')
