...
function VideoOptionsResolutionPanel_OnEvent (self, event, ...)
	...
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		...
		if GetCVar("uiscale") and tonumber(GetCVar("uiscale")) > 0.91 then
			VideoOptionsResolutionPanel_Default(VideoOptionsResolutionPanel, true)
			StaticPopup_Show("VIDEO_OPTIONS_RESET_UISCALE_POPUP")
		end
	end
end
