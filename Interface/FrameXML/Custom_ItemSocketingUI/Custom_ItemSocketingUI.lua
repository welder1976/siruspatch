...
hooksecurefunc("SocketInventoryItem", function( slotID )
	ItemSocketingFrame.containerID = -1
	ItemSocketingFrame.slotID = slotID
end)

hooksecurefunc("SocketContainerItem", function( containerID, slotID )
	ItemSocketingFrame.containerID = containerID
	ItemSocketingFrame.slotID = slotID
end)
	...
	if ( IsModifiedClick() ) then
		...
		if IsControlKeyDown() then
			if existingSocketLink then
				...
				if quality == 5 then
					SendServerMessage("ACMSG_REMOVE_SOCKET_FROM_ITEM", string.format("%d:%d:%d", self:GetParent().containerID, self:GetParent().slotID, self:GetID()))
				end
			end
		else
			HandleModifiedItemClick(newSocketLink or existingSocketLink)
		end
	else
