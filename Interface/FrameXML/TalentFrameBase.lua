...
function TalentFrame_Update(TalentFrame)
	...
	-- if PlayerTalentFrame and PlayerTalentFrame.previewState then
	-- 	isActiveTalentGroup = false
	-- end
	if PlayerTalentFrame and (PlayerTalentFrame.resetTalents or PlayerTalentFrame.learnPreviewTalentsAnimation or PlayerTalentFrame.previewState) then
		isActiveTalentGroup = false
		if PlayerTalentFrame.learnPreviewTalentsAnimation then
			firstUnlock = false
		end
	else
		if firstUnlock == false then
			firstUnlock = true
		else
			firstUnlock = nil
		end
	...
	if firstUnlock then
		PlayerTalentFrameResetCVar()

		-- for i = 1, 3 do
		-- 	local frame = _G["PlayerTalentFramePanel"..i]

		-- 	if frame then
		-- 		-- frame.GlowFrame1.inAnim:Play()
		-- 		-- frame.GlowFrame2.inAnim:Play()
		-- 		-- frame.GlowFrame3.inAnim:Play()
		-- 		-- frame.GlowFrame4.inAnim:Play()
		-- 		-- frame.GlowFrame5.inAnim:Play()
		-- 	end
		-- end
	end
	...
	for i=1, MAX_NUM_TALENTS do
		...
		if ( i <= numTalents ) then
			...
			if ( name and tier <= MAX_NUM_TALENT_TIERS) then
				...
				button.isExceptional = isExceptional
				...
				if (goldBorder and button.GoldBorder) then
					button.GoldBorder:SetShown(C_Talent.GetSelectedTalentGroup() < 3)
					button.EtherealGoldBorder:SetShown(C_Talent.GetSelectedTalentGroup() > 2)
					button.Slot:Hide();
					button.SlotShadow:Hide();
				else
					if (button.GoldBorder) then
						button.GoldBorder:Hide();
						button.EtherealGoldBorder:Hide()
					end
					button.Slot:Show();
				...
				local prereqsSet = TalentFrame_SetPrereqs(TalentFrame, tier, column, forceDesaturated, tierUnlocked, preview,
				...
				if ( prereqsSet and ((preview and meetsPreviewPrereq) or (not preview and meetsPrereq)) ) then
                    ...
                    button.EtherealGoldBorder:SetDesaturated(nil)

					if ( displayRank < maxRank and not PlayerTalentFrame.resetTalents and not PlayerTalentFrame.learnPreviewTalentsAnimation ) then
					...
					button._isDisable = false
				else
					SetItemButtonDesaturated(button, 1);
					button:SetPushedTexture(nil);
					button:SetHighlightTexture(nil);
					button.GoldBorder:SetDesaturated(1);
                    button.EtherealGoldBorder:SetDesaturated(1)
