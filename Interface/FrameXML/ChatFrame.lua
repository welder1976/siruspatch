...
SlashCmdList["USE_TALENT_SPEC"] = function(msg)
	...
	if ( group ) then
		...
		if ( groupNumber ) then
			C_Talent.SetActiveTalentGroup(groupNumber)
		end
	end
end
...
function ChatFrame_GetOriginalChannelName( chatTarget )
	local channelID, localeChannelName = GetChannelName(chatTarget)
	local channelName, header, _, channelNumber

	for i = 1, GetNumDisplayChannels() do
		channelName, header, _, channelNumber = GetChannelDisplayInfo(i)

		if not header then
			if channelNumber == channelID then
				break
			end
		end
	end

	if channelName ~= localeChannelName then
		return channelName
	else
		return localeChannelName
	end
end
...
function ChatFrame_MessageEventHandler(self, event, ...)
	if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
		...
		if ( type == "SYSTEM" or type == "SKILL" or type == "LOOT" or type == "MONEY" or
			...
			if type == "SYSTEM" then
				local name, ilevel, msg = string.match(arg1, "%|Hplayer%:(.-)%((%d+)%)%|h%[.-%]%|h%:(.*)")

				if name and ilevel and msg then
					arg1 = string.format(WHO_LIST_FORMAT_NEW, name, name, msg, ilevel)
				end

				--Add Timestamps for system messages
				if ( CHAT_TIMESTAMP_FORMAT ) then
					arg1 = strconcat(BetterDate(CHAT_TIMESTAMP_FORMAT, time()), arg1);
				end
			end
...
function ChatFrame_OnUpdate(self, elapsedSec)
	local flash = self.buttonFrame2.ScrollToBottomButton.Flash
...
function ChatEdit_SendText(editBox, addHistory)
	...
	editBox.language = editBox.language or GetDefaultLearnLanguage()
...
function LanguageMenu_UpdateLanguage(self)
	self:Hide()
	UIMenu_Initialize(self)
...
function GetDefaultLearnLanguage()
	...
	for i = 1, GetNumLanguages(), 1 do
		...
		if language == _language then
			knowLanguage = true
		end
	end

	return not knowLanguage and GetLanguageByIndex(1) or language
end

function SetLanguageForAllChatTabs( language, setForDefault )
	for i = 1, FCF_GetNumActiveChatFrames() do
		local chatFrame = _G["ChatFrame"..i]

		chatFrame.editBox.language = language

		if setForDefault then
			chatFrame.defaultLanguage = language
		end
	end
...
function LanguageMenu_LoadLanguages(self)
	...
	for i = 1, numLanguages, 1 do
		...
		UIMenu_AddButton(self, _language, nil, LanguageMenu_Click)
	end

	language = not knowLanguage and GetLanguageByIndex(1) or language

	SetLanguageForAllChatTabs(language, setForDefault)
...
function ChatChannelDropDown_PopOutChat(self, chatType, chatTarget)
	...
	if ( chatType == "BN_CONVERSATION" ) then
		...
		if ( chatType == "CHANNEL" ) then
			windowName = Chat_GetChannelShortcutName(ChatFrame_GetOriginalChannelName(chatTarget), true);
		else
		...
		if ( chatType == "CHANNEL" ) then
			...
			ChatFrame_AddChannel(frame, Chat_GetChannelShortcutName(ChatFrame_GetOriginalChannelName(chatTarget)));
		end

		--Remove the things popped out from the source chat frame.
		if ( chatType == "CHANNEL" ) then
			ChatFrame_RemoveChannel(sourceChatFrame, windowName);
		else
...
function Chat_GetChannelShortcutName(target, isOriginal)
	local _, name = nil, target

	if tonumber(name) then
		_, name = GetChannelName(name)
	end

	name = strtrim(name:match("([^%-]+)"))

	return not isOriginal and name:gsub("%s", "") or name
end
...
function GetItemEntryByMsgLoot( msg )
	if string.find(msg, string.Left(LOOT_ITEM_SELF, 21)) or string.find(msg, string.Left(LOOT_ITEM_CREATED_SELF, 21)) or string.find(msg, string.Left(LOOT_ITEM_PUSHED_SELF, 21)) then
		...
		if not itemEntry then
			itemEntry = string.match(msg, "|Hitem:(%d+)")
		end
...
local discordColor = CreateColor(0.73,0.58,0.88)
local httpColor = CreateColor(0.34,0.7,0.39)
...
local ChatLinksData = {
	-- ID: 1
	["discord.gg"] = function(scheme, domain, path, query, hash)
		-- Invite link
		if scheme == "https" and query == "" and hash == "" then
			if path:len() == 6 and path:match("%w%w%w%w%w%w") then
				return (discordColor:GenerateHexColorMarkup().."|Hurl:1:%s:0:0|h[Discord]|h|r"):format(path)
			end
		end
		return false
	end,
	-- ID: 2
	["sirus.su"] = function(scheme, domain, path, query, hash)
		return false
	end,
	-- ID: 3
	["forum.sirus.su"] = function(scheme, domain, path, query, hash)
		--===================--
		--== Threads | 1,2 ==--
		--===================--
		if path:match("^threads/") then
			local title, id, post = path:match("^threads/(.-).(%d+)/(.*)")
			post = (post or ""):match("post%-(%d+)")
			return (httpColor:GenerateHexColorMarkup().."|Hurl:3:%s:%s:%s|h[Форум: %s]|h|r"):format(not post and 1 or 2, id, post or 0, title:gsub("^%l", string.upper):gsub("%-", " "))
		--===================--
		--== Members | 3   ==--
		--===================--
		elseif path:match("^members/") then
			local title, id = path:match("^members/(.-).(%d+)/")
			return (httpColor:GenerateHexColorMarkup().."|Hurl:3:3:%s:0|h[Форум: %s]|h|r"):format(id, title:gsub("^%l", string.upper):gsub("%-", " "))
		--===================--
		--== Forums | 4    ==--
		--===================--
		elseif path:match("^forums/") then
			local title, id = path:match("^forums/(.-).(%d+)/")
			return (httpColor:GenerateHexColorMarkup().."|Hurl:3:4:%s:0|h[Форум: %s]|h|r"):format(id, title:gsub("^%l", string.upper):gsub("%-", " "))
		--===================--
		--== Whats new | 5 ==--
		--===================--
		elseif path:match("^whats%-new/") then
			return httpColor:GenerateHexColorMarkup().."|Hurl:3:5:0:0|h[Форум: Что нового]|h|r"
		else
			--===================--
			--== Forum | 0     ==--
			--===================--
			if path == "" then
				return httpColor:GenerateHexColorMarkup().."|Hurl:3:0:0:0|h[Форум]|h|r"
			end
		end
...
function ParseURLsInText(msg)
	for scheme, domain, path in msg:gmatch("(%w+)://(.-)/(%S*)") do
		local pathRaw = path

		local hash = path:match("#(.*)") or ""
		path = path:gsub("#(.*)", "")

		local query = path:match("?(.*)") or ""
		path = path:gsub("?(.*)", "")

		if ChatLinksData[domain] then
			local link = ChatLinksData[domain](scheme, domain, path, query, hash)
			if link then
				msg = msg:replace(("%s://%s/%s"):format(scheme, domain, pathRaw), link)
			end
		end
	end
