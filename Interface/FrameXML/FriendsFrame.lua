...
function FriendsFrame_DataAvailabilityCheck()
	local numFriends = GetNumFriends()
	local friendsCounter = 0
	...
	FriendsListFrame.Spinner:SetShown(numFriends > 0 and friendsCounter ~= numFriends)
end
...
function FriendsList_Update()
	...
	if ( not FriendsListFrame:IsShown() and FriendsListFrame.Spinner:IsShown() ) then
		return;
	end
...
function WhoList_Update()
	...
	local button, buttonText, classTextColor, classFileName, ilevel;
	local columnTable;
	...
	for i=1, WHOS_TO_DISPLAY, 1 do
		...
		name, guild, level, race, class, zone, classFileName, ilevel = GetWhoInfo(whoIndex);
		...
		buttonText = _G["WhoFrameButton"..i.."Ilvl"];
		...
		if ( showScrollBar ) then
			variableText:SetWidth(90);
		else
			variableText:SetWidth(100);
		end
	...
	-- If need scrollbar resize columns
	if ( showScrollBar ) then
		WhoFrameColumn_SetWidth(WhoFrameColumnHeader2, 100);
		UIDropDownMenu_SetWidth(WhoFrameDropDown, 80);
	else
		WhoFrameColumn_SetWidth(WhoFrameColumnHeader2, 110);
		UIDropDownMenu_SetWidth(WhoFrameDropDown, 90);
	end
...
function GuildControlPopupFrame_OnShow()
	...
	UIDropDownMenu_SetText(GuildControlPopupFrameDropDown, GuildControlGetRankName(rankID))
...
function FriendsFrame_SetButton(button, index, firstButton)
	...
	button:SetShown(not FriendsListFrame.Spinner:IsShown())

	if nameText and nameColor and infoText then
...
function FriendsFrameStatusDropDown_ShowTooltip()
	...
	elseif ( status == 4 ) then
		statusText = FRIENDS_LIST_INVISIBLE;
	else
