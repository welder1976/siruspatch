...
function WatchFrame_OnUpdate(self, elapsed)
	...
	WatchFrameLines.AchievementHeader:SetShown(WATCHFRAME_ACHIEVEMENTLINES[1])
end
...
function WatchFrame_Collapse (self)
	...
	local button = WatchFrameHeader.MinimizeButton;
...
function WatchFrame_Update (self)
	...
	if ( totalObjectives > 0 ) then
		...
		WatchFrameHeader.MinimizeButton:Show();
		...
		if ( totalOffset < WATCHFRAME_INITIAL_OFFSET ) then
			WatchFrameHeader.MinimizeButton:Enable();
		else
			WatchFrameHeader.MinimizeButton:Disable();
		end
	else
		WatchFrameHeader:Hide();
		WatchFrameHeader.MinimizeButton:Hide();
	end
	...
	WatchFrame:SetShown(not IsActiveBattlefieldArena())
end
...
function WatchFrame_SetLine(line, anchor, verticalOffset, isHeader, text, dash, hasItem, isComplete, questID)
	line.Icon:Hide()
	...
	if ( line.text:GetHeight() > WATCHFRAME_LINEHEIGHT ) then
		if ( isComplete ) then
			if not isHeader then
				if not WATCHFRAME_QUESTDATA[questID] then
					line.Glow.Anim:Play()
					...
					local _line = WatchFrame_GetQuestLine();
					WatchFrame_SetLine(_line, line, WATCHFRAMELINES_FONTSPACING, false, QUEST_WATCH_QUEST_READY, DASH_SHOW, nil, true, questID)

					WATCHFRAME_QUESTDATA[questID] = true

					line:SetHeight(line.text:GetHeight() + 4)
				else
					line.text:SetText(QUEST_WATCH_QUEST_READY)
				end
			end
		...
		if isComplete and text == QUEST_WATCH_QUEST_READY then
			line.dash:Hide()
			line.text:SetTextColor(0.6, 0.6, 0.6)
		end
	end
		...
		for i = 1, numTrackedAchievements do
			...
			if ( not completed and (not displayOnlyArena) or achievementCategory == WATCHFRAME_ACHIEVEMENT_ARENA_CATEGORY ) then
				...
				if ( not previousLine ) then
					WatchFrameLines.AchievementHeader:SetFrameLevel(line:GetFrameLevel() - 1)
					WatchFrameLines.AchievementHeader.Text:SetText(ACHIEVEMENT_BUTTON)

					if GetNumQuestWatches() > 0 then
						WatchFrameLines.AchievementHeader:SetPoint("TOPRIGHT", lineFrame, "TOPRIGHT", 16, initialOffset)
						initialOffset = initialOffset - 34
					else
						WatchFrameLines.AchievementHeader:SetPoint("TOPRIGHT", lineFrame, "TOPRIGHT", 16, 29)
					end
...
function WatchFrame_DisplayTrackedQuests (lineFrame, initialOffset, maxHeight, frameWidth)
	...
	for i = 1, numQuestWatches do
		...
		if ( questIndex ) then
			...
			if ( filterOK ) then
				...
				if ( isComplete ) then
					local compliteText = GetQuestLogCompletionText(questIndex)
					if compliteText and compliteText ~= "" then
						line = WatchFrame_GetQuestLine();
						WatchFrame_SetLine(line, lastLine, WATCHFRAMELINES_FONTSPACING, not IS_HEADER, GetQuestLogCompletionText(questIndex), DASH_SHOW, nil, true, questID);
						lastLine = line;
					end
				else
					for j = 1, numObjectives do
						text, _, finished = GetQuestLogLeaderBoard(j, questIndex);
						if ( not finished and text and text ~= "" ) then
							text = WatchFrame_ReverseQuestObjective(text);
							line = WatchFrame_GetQuestLine();
							WatchFrame_SetLine(line, lastLine, WATCHFRAMELINES_FONTSPACING, not IS_HEADER, text, DASH_SHOW, item, nil, questID);
							lastLine = line;
						end
					end
	...
	if #questIDBuffer > 0 then
		local removeQuestID = difference(questIDList, questIDBuffer)
		if removeQuestID then
			if WATCHFRAME_QUESTDATA[removeQuestID] then
				WATCHFRAME_QUESTDATA[removeQuestID] = nil
			end
		end
	end

	questIDBuffer = CopyTable(questIDList)
		...
		if ( line ) then
			if ( index == self.startLine ) then
				...
				if ( onEnter ) then
					...
					if WATCHFRAME_QUESTDATA[line.questID] then
						line.text:SetTextColor(0.6, 0.6, 0.6)
					else
						line.text:SetTextColor(0.8, 0.8, 0.8);
					end
...
function WatchFrame_SetWidth(width)
	WATCHFRAME_EXPANDEDWIDTH = 234
	WATCHFRAME_MAXLINEWIDTH = 232
...
function ObjectiveTracker_Collapse( self, ... )
	WatchFrame.collapsed = true
	WatchFrame.BlocksFrame:Hide()
	WatchFrame.HeaderMenu.MinimizeButton:GetNormalTexture():SetTexCoord(0, 0.5, 0, 0.5)
	WatchFrame.HeaderMenu.MinimizeButton:GetPushedTexture():SetTexCoord(0.5, 1, 0, 0.5)
	WatchFrame.HeaderMenu.Title:Show()
end

function ObjectiveTracker_Expand( self, ... )
	WatchFrame.collapsed = nil
	WatchFrame.BlocksFrame:Show()
