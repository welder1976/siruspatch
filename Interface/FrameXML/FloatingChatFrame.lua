...
CHAT_FRAME_TEXTURES = {
	...
	"ButtonFrame2Background",
	"ButtonFrame2TopLeftTexture",
	"ButtonFrame2BottomLeftTexture",
	"ButtonFrame2TopRightTexture",
	"ButtonFrame2BottomRightTexture",
	"ButtonFrame2LeftTexture",
	"ButtonFrame2RightTexture",
	"ButtonFrame2BottomTexture",
	"ButtonFrame2TopTexture",
}
...
function FloatingChatFrame_OnMessageScrollChanged( self, ... )
...
function FloatingChatFrame_UpdateScroll( self )
	...
	self.buttonFrame2.ScrollBar:SetShown(isShown)

	if isShown then
		self.buttonFrame2.ScrollBar:SetMinMaxValues(1, numMessages)
		self.buttonFrame2.ScrollBar:SetValue(numMessages - self:GetCurrentScroll())
...
function FCF_UpdateScrollbarAnchors(chatFrame)
	if chatFrame.buttonFrame2.ScrollBar then
		chatFrame.buttonFrame2.ScrollBar:ClearAllPoints();
		chatFrame.buttonFrame2.ScrollBar:SetPoint("TOP", chatFrame.buttonFrame2, 0, 0);

		chatFrame.buttonFrame2.ScrollToBottomButton:ClearAllPoints()

		if chatFrame.resizeButton:IsShown() then
			chatFrame.buttonFrame2.ScrollToBottomButton:SetPoint("BOTTOM", chatFrame.resizeButton, "TOP", -5, 0)
		else
			chatFrame.buttonFrame2.ScrollToBottomButton:SetPoint("CENTER", chatFrame.resizeButton, -5, 6)
		end

		if chatFrame.buttonFrame2.ScrollToBottomButton:IsShown() then
			chatFrame.buttonFrame2.ScrollBar:SetPoint("BOTTOM", chatFrame.buttonFrame2.ScrollToBottomButton, "TOP", 0, 0);
		elseif chatFrame.resizeButton:IsShown() then
			chatFrame.buttonFrame2.ScrollBar:SetPoint("BOTTOM", chatFrame.resizeButton, "TOP", 0, 0);
		else
			chatFrame.buttonFrame2.ScrollBar:SetPoint("BOTTOMLEFT", chatFrame.buttonFrame2, "BOTTOMRIGHT", 0, 0);
		end
	end
end
...
function FloatingChatFrame_UpdateBackgroundAnchors(self)
	...
	if self.buttonFrame2.ScrollBar then
		scrollbarWidth = self.buttonFrame2.ScrollBar:GetWidth();
	end
	...
	_G[self:GetName().."Background"]:SetPoint("TOPLEFT", self, "TOPLEFT", -2, 3 + quickButtonHeight);
	_G[self:GetName().."Background"]:SetPoint("TOPRIGHT", self, "TOPRIGHT", 2 + scrollbarWidth, 3 + quickButtonHeight);
	_G[self:GetName().."Background"]:SetPoint("BOTTOMLEFT", self, "BOTTOMLEFT", -2, -6);
	_G[self:GetName().."Background"]:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", 2 + scrollbarWidth, -6);
...
function FCF_OnUpdate(elapsed)
	...
	for _, frameName in pairs(CHAT_FRAMES) do
		...
		if ( chatFrame and chatFrame:IsShown() ) then
			...
			if ( MOVING_CHATFRAME or 
				chatFrame.resizeButton:GetButtonState() == "PUSHED" or 
				(chatFrame.isDocked and GENERAL_CHAT_DOCK.overflowButton.list:IsShown()) or 
				(chatFrame.buttonFrame2.ScrollBar and chatFrame.buttonFrame2.ScrollBar:IsMouseOver()) or
				(chatFrame.buttonFrame2.ScrollToBottomButton and chatFrame.buttonFrame2.ScrollToBottomButton:IsMouseOver())) then	
				overSomething = true;
			...
			elseif ( chatFrame:IsMouseOver(topOffset, -2, -2, 2) or	--This should be slightly larger than the hit rect insets to give us some wiggle room.
				...
				(chatFrame.buttonFrame:IsMouseOver()) or 
				(chatFrame.buttonFrame2.ScrollBar and chatFrame.buttonFrame2.ScrollBar:IsMouseOver()) or
				(chatFrame.buttonFrame2.ScrollToBottomButton and chatFrame.buttonFrame2.ScrollToBottomButton:IsMouseOver())) then
				overSomething = true;
				chatFrame.mouseOutTime = 0;
...
function FCF_SetButtonSide(chatFrame, buttonSide, forceUpdate)
	...
	chatFrame.buttonFrame2:ClearAllPoints()

	chatFrame.buttonFrame2:SetPoint("TOPLEFT", chatFrame, "TOPRIGHT", 4, 0)
	chatFrame.buttonFrame2:SetPoint("BOTTOMLEFT", chatFrame, "BOTTOMRIGHT", 4, 0)
	...
	elseif ( buttonSide == "right" ) then
		chatFrame.buttonFrame:SetPoint("TOPLEFT", chatFrame.buttonFrame2, "TOPRIGHT", 4, topY);
		chatFrame.buttonFrame:SetPoint("BOTTOMLEFT", chatFrame.buttonFrame2, "BOTTOMRIGHT", 4, 0);
	end
...
function FCF_FadeInScrollbar(chatFrame)
	if chatFrame.buttonFrame2.ScrollBar and chatFrame.buttonFrame2.ScrollBar:IsShown() then
		UIFrameFadeIn(chatFrame.buttonFrame2.ScrollBar, CHAT_FRAME_FADE_TIME, chatFrame.buttonFrame2.ScrollBar:GetAlpha(), .6)

		if chatFrame.buttonFrame2.ScrollToBottomButton then
			UIFrameFadeIn(chatFrame.buttonFrame2.ScrollToBottomButton, .1, chatFrame.buttonFrame2.ScrollToBottomButton:GetAlpha(), 1)
		end
	end
end

function FCF_FadeOutScrollbar(chatFrame)
	if chatFrame:GetCurrentScroll() > 0 then
		return
	end
	
	if chatFrame.buttonFrame2.ScrollBar and chatFrame.buttonFrame2.ScrollBar:IsShown() then
		UIFrameFadeOut(chatFrame.buttonFrame2.ScrollBar, CHAT_FRAME_FADE_OUT_TIME, chatFrame.buttonFrame2.ScrollBar:GetAlpha(), 0)

		if chatFrame.buttonFrame2.ScrollToBottomButton then
			if UIFrameIsFlashing(chatFrame.buttonFrame2.ScrollToBottomButton.Flash) then
				UIFrameFadeRemoveFrame(chatFrame.buttonFrame2.ScrollToBottomButton)
				chatFrame.buttonFrame2.ScrollToBottomButton:SetAlpha(1)
			else
				UIFrameFadeOut(chatFrame.buttonFrame2.ScrollToBottomButton, CHAT_FRAME_FADE_OUT_TIME, chatFrame.buttonFrame2.ScrollToBottomButton:GetAlpha(), 0)
			end
		end
	end
end