...
function BuffButton_OnClick(self, button)
	GMAuraOnClickHandler(self, button)
	CancelUnitBuff(self.unit, self:GetID(), self.filter);
end

function DebuffButton_OnClick( self, button )
	GMAuraOnClickHandler(self, button)
end
...
function BuffFrame_UpdateAllBuffAnchors()
	...
	if ( DEBUFF_ACTUAL_DISPLAY > 0 ) then
		bottomEdgeExtent = bottomEdgeExtent + BUFF_BUTTON_HEIGHT + ceil(DEBUFF_ACTUAL_DISPLAY / BUFFS_PER_ROW) * (BUFF_BUTTON_HEIGHT + BUFF_ROW_SPACING);
	else
...
function GMAuraOnClickHandler( self, button )
	if IsShiftKeyDown() and IsGMAccount() and button == "RightButton" then
		local _, _, _, _, _, _, _, _, _, _, spellID

		if string.find(string.upper(self:GetName()), "DEBUFF") then
			_, _, _, _, _, _, _, _, _, _, spellID = UnitDebuff(self.unit, self:GetID())
		else
			_, _, _, _, _, _, _, _, _, _, spellID = UnitBuff(self.unit, self:GetID())
		end
