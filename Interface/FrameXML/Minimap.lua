...
function MinimapMailFrameUpdate()
	...
	sender1 = sender1 == UnitName("player") and MAILFRAME_BLACK_MARKET or sender1
	sender2 = sender2 == UnitName("player") and MAILFRAME_BLACK_MARKET or sender2
	sender3 = sender3 == UnitName("player") and MAILFRAME_BLACK_MARKET or sender3
...
function MiniMapBattlefieldFrameMixin:OnLoad()
	self:RegisterForClicks("LeftButtonUp", "RightButtonUp")
	self:SetFrameLevel(self:GetFrameLevel() + 1)
	self.timeInQueue = 0
end

function MiniMapBattlefieldFrameMixin:OnEnter()
	GameTooltip:SetOwner(self, "ANCHOR_LEFT")
...
function MiniMapBattlefieldFrameMixin:OnUpdate()
...
function MiniMapBattlefieldFrameMixin:OnLeave()
	GameTooltip:Hide()
	self:SetScript("OnUpdate", nil)
end

function MiniMapBattlefieldFrameMixin:OnClick( button )
	GameTooltip:Hide()
...
function MiniMapBattlefieldFrameMixin:UpdateArt()
	local _, _, _, _, _, _, registeredMatch = GetBattlefieldStatus(1)
	...
	if registeredMatch or isRegistered then
		...
		atlasKey = UnitFactionGroup("player") or "Alliance"
	end

	if atlasKey == "Neutral" then
		return
	end

	atlasKey = "bfa-landingbutton-"..string.lower(atlasKey)

	local info = C_Texture.GetAtlasInfo(atlasKey.."-up")
	self:SetSize(info and info.width or 0, info and info.height or 0)

	self.NormalTexture:SetAtlas(atlasKey.."-up", true)
	self.PushedTexture:SetAtlas(atlasKey.."-down", true)
	self.HighlightTexture:SetAtlas(atlasKey.."-highlight", true)
end