...
function GetFlyoutInfo( floyutSpellID )
	if FLYOUT_STORAGE[floyutSpellID] then
		local name 			= GetSpellInfo(floyutSpellID)
		local description 	= ""
		local numSlots 		= #FLYOUT_STORAGE[floyutSpellID]
		local isKnown 		= IsSpellKnown(floyutSpellID)

		return name, description, numSlots, isKnown
	end
end

function GetFlyoutSlotInfo( floyutSpellID, index )
	if FLYOUT_STORAGE[floyutSpellID] and FLYOUT_STORAGE[floyutSpellID][index] then
		local spellID 			= FLYOUT_STORAGE[floyutSpellID][index]
		local overrideSpellID 	= spellID
...
function SpellFlyout_Toggle(self, spellID, parent, direction, distance, isActionBar, specID, showFullTooltip, reason)
	...
	local numButtons = 0
	for i=1, numSlots do
		local spellID, overrideSpellID, isKnown, spellName, slotSpecID = GetFlyoutSlotInfo(spellID, i)
		local visible = true
		...
		if ( ((not offSpec or slotSpecID == 0) and visible and isKnown) or (offSpec and slotSpecID == specID) ) then
			...
			local _, _, icon = GetSpellInfo(overrideSpellID)
