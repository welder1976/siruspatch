...
function AuctionHouseTablePriceDisplayMixin:Init(owner)
	...
	if self.Checkmark then
		...
		self.Checkmark:SetPoint("LEFT", self.MoneyDisplay, "RIGHT", 6, 0);
	end
end
...
function AuctionHouseTableCellFavoriteMixin:OnShow()
	self:RegisterCustomEvent("AUCTION_HOUSE_FAVORITES_UPDATED");
end

function AuctionHouseTableCellFavoriteMixin:OnHide()
	self:UnregisterCustomEvent("AUCTION_HOUSE_FAVORITES_UPDATED");
end
...
function AuctionHouseTableCellFavoriteButtonMixin:UpdateFavoriteState()
	...
	if isFavorite then
		...
		local defaultTexture = self.textureLocked and "auctionhouse-icon-favorite-off";
		if defaultTexture then
			self.NormalTexture:SetAtlas(defaultTexture);
		else
...
function AuctionHouseTableCellAllAuctionsPriceMixin:OnEvent(event, ...)
	if event == "OWNED_AUCTION_BIDDER_INFO_RECEIVED" then
		...
		if auctionID == self.rowData.auctionID then
			...
			self:UnregisterCustomEvent("OWNED_AUCTION_BIDDER_INFO_RECEIVED");
	...
	if rowData.bidder then
		if rowData.bidder == "" then
			...
			if updatedName then
				...
				self:RegisterCustomEvent("OWNED_AUCTION_BIDDER_INFO_RECEIVED");
			end
		end
	end
end
...
AuctionHouseTableCellAuctionsOwnersMixin = CreateFromMixins(AuctionHouseTableCellOwnersMixin, AuctionHouseTableCellAuctionsTextMixin);
...
function AuctionHouseTableCellLevelMixin:UpdateDisplay()
	...
	if not extraInfo then
		self.Text:SetText("");
		self:RegisterCustomEvent("EXTRA_BROWSE_INFO_RECEIVED");
		return;
	end

	self:UnregisterCustomEvent("EXTRA_BROWSE_INFO_RECEIVED");
...
function AuctionHouseTableCellItemQuantityMixin:Init(owner, hideBidStatus)
	...
	self.Text:SetJustifyH(self:GetAttribute("justificationH") or "LEFT");
end
...
function AuctionHouseTableHeaderStringMixin:OnLoad()
...
function AuctionHouseTableBuilder.GetItemSellListLayout(owner, itemList, isEquipment)
