...
function AuctionHouseSellFrameAlignedControlMixin:OnLoad()
	self:SetLabel(_G[self:GetAttribute("labelText")]);
end
...
AuctionHouseAlignedQuantityInputBoxMixin = CreateFromMixins(AuctionHouseQuantityInputBoxMixin);

function AuctionHouseAlignedQuantityInputBoxMixin:OnLoad()
	AuctionHouseQuantityInputBoxMixin.OnLoad(self)
end
...
AuctionHouseAlignedQuantityInputFrameMixin = CreateFromMixins(AuctionHouseSellFrameAlignedControlMixin);

function AuctionHouseAlignedQuantityInputFrameMixin:OnLoad()
...
AuctionHouseAlignedPriceInputFrameMixin = CreateFromMixins(AuctionHouseSellFrameAlignedControlMixin);

function AuctionHouseAlignedPriceInputFrameMixin:OnLoad()
	AuctionHouseSellFrameAlignedControlMixin.OnLoad(self);

	self.PerItemPostfix:SetPoint("LEFT",self.MoneyInputFrame, "RIGHT", 4, 4);
end
...
AuctionHouseAlignedDurationDropDownMixin = CreateFromMixins(AuctionHouseSellFrameAlignedControlMixin);

function AuctionHouseAlignedDurationDropDownMixin:OnLoad()
	AuctionHouseSellFrameAlignedControlMixin.OnLoad(self);
end
...
AuctionHouseAlignedPriceDisplayMixin = CreateFromMixins(AuctionHouseSellFrameAlignedControlMixin);

function AuctionHouseAlignedPriceDisplayMixin:OnLoad()
	AuctionHouseSellFrameAlignedControlMixin.OnLoad(self);

	self.MoneyDisplayFrame:ClearAllPoints();
	self.MoneyDisplayFrame:SetPoint("LEFT", self.Label, "RIGHT", 18, 0);
	self.MoneyDisplayFrame:SetPoint("RIGHT");
end
...
AuctionHouseSellFrameItemDisplayMixin = CreateFromMixins(AuctionHouseInteractableItemDisplayMixin);
...
AuctionHouseSellFrameMixin = CreateFromMixins(AuctionHouseSortOrderSystemMixin, LayoutMixin, VerticalLayoutMixin, AuctionHouseBackgroundMixin);

local AUCTION_HOUSE_SELL_FRAME_EVENTS = {
	"CURSOR_UPDATE",
}

local AUCTION_HOUSE_SELL_FRAME_CUSTOM_EVENTS = {
...
function AuctionHouseSellFrameMixin:OnLoad()
	...
	self.CreateAuctionTabLeft:SetAtlas("auctionhouse-selltab-left", true);
	self.CreateAuctionTabMiddle:SetAtlas("auctionhouse-selltab-middle");
	self.CreateAuctionTabRight:SetAtlas("auctionhouse-selltab-right", true);

	self.PostButton:SetPoint("LEFT", self.Label, "RIGHT", 12, 0);
...
function AuctionHouseSellFrameMixin:OnShow()
	...
	FrameUtil.RegisterFrameForCustomEvents(self, AUCTION_HOUSE_SELL_FRAME_CUSTOM_EVENTS);
...
function AuctionHouseSellFrameMixin:GetDefaultPrice()
	...
	if isValid then
		...
		local vendorPrice = select(11, GetItemInfo(isValid));
		if vendorPrice then
