...
AuctionHouseBuyDialogBuyNowButtonMixin = CreateFromMixins(AuctionHouseBuyDialogButtonMixin, ButtonWithDisableMixin);
...
local AUCTION_HOUSE_BUY_DIALOG_CUSTOM_EVENTS = {
...
function AuctionHouseBuyDialogMixin:OnLoad()
	local framelevel = self:GetFrameLevel();
	self.Border:SetFrameLevel(framelevel);
	self.DialogOverlay:SetFrameLevel(framelevel);

	self.DialogOverlay:SetPoint("TOPLEFT", self:GetParent(), "TOPLEFT", 0, -22);
	self.DialogOverlay:SetPoint("BOTTOMRIGHT", self:GetParent(), "BOTTOMRIGHT");

	self.TimeLeftText:SetPoint("CENTER", self.CancelButton, "CENTER");
end

function AuctionHouseBuyDialogMixin:OnShow()
	FrameUtil.RegisterFrameForCustomEvents(self, AUCTION_HOUSE_BUY_DIALOG_CUSTOM_EVENTS);
end

function AuctionHouseBuyDialogMixin:OnHide()
	FrameUtil.UnregisterFrameForCustomEvents(self, AUCTION_HOUSE_BUY_DIALOG_CUSTOM_EVENTS);
