...
local ITEM_AUCTIONS_SCROLL_OFFSET_REFRESH_THRESHOLD = 30;
local BIDS_TAB_ID = 2;

AuctionHouseAuctionsSummaryListMixin = CreateFromMixins(ScrollListMixin, AuctionHouseBackgroundMixin);
...
AuctionHouseAuctionsSummaryLineMixin = CreateFromMixins(ScrollListLineMixin);
...
local AUCTIONS_FRAME_CUSTOM_EVENTS = {
...
function AuctionHouseAuctionsFrameMixin:OnShow()
	FrameUtil.RegisterFrameForCustomEvents(self, AUCTIONS_FRAME_CUSTOM_EVENTS);
...
function AuctionHouseAuctionsFrameMixin:OnHide()
	FrameUtil.UnregisterFrameForCustomEvents(self, AUCTIONS_FRAME_CUSTOM_EVENTS);
end
...
function AuctionHouseAuctionsFrameMixin:SelectItemKey(itemKey)
	...
	if itemKeyInfo.isCommodity then
		...
		local function ItemBuyListRefreshCallback(lastDisplayEntry)
			...
			if numEntries > 0 and numEntries - lastDisplayEntry < ITEM_AUCTIONS_SCROLL_OFFSET_REFRESH_THRESHOLD then
