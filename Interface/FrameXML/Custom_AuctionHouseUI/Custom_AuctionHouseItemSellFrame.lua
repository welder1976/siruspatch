...
AuctionHouseItemSellFrameMixin = CreateFromMixins(AuctionHouseSellFrameMixin, LayoutMixin, VerticalLayoutMixin, AuctionHouseBackgroundMixin);

local AUCTION_HOUSE_ITEM_SELL_FRAME_CUSTOM_EVENTS = {
...
function AuctionHouseItemSellFrameMixin:OnShow()
	...
	FrameUtil.RegisterFrameForCustomEvents(self, AUCTION_HOUSE_ITEM_SELL_FRAME_CUSTOM_EVENTS);
...
function AuctionHouseItemSellFrameMixin:OnHide()
	...
	FrameUtil.UnregisterFrameForCustomEvents(self, AUCTION_HOUSE_ITEM_SELL_FRAME_CUSTOM_EVENTS);
...
function AuctionHouseItemSellFrameMixin:OnEvent(event, ...)
	...
	elseif event == "AUCTION_MULTISELL_START" then
		...
		local itemTexture = select(10, GetItemInfo(isValid));
...
function AuctionHouseItemSellFrameMixin:InitializeItemSellList()
	...
	itemSellList:SetTableBuilderLayout(AuctionHouseTableBuilder.GetItemSellListLayout(self, itemSellList, isEquipment));
...
function AuctionHouseItemSellFrameMixin:SetItem(itemLocation, fromItemDisplay, refreshListWithPreviousItem)
	...
	if itemKey then
		...
		itemSellList:SetTableBuilderLayout(AuctionHouseTableBuilder.GetItemSellListLayout(self, itemSellList, itemKeyInfo and itemKeyInfo.isEquipment));
	else
...
function AuctionHouseItemSellFrameMixin:CanPostItem()
	...
	if bidPrice and bidPrice > MAXIMUM_BID_PRICE or buyoutPrice and buyoutPrice > MAXIMUM_BID_PRICE then
