...
function AuctionHouseItemListLineMixin:OnLoad()
...
AuctionHouseFavoritableLineMixin = CreateFromMixins(AuctionHouseItemListLineMixin);

function AuctionHouseFavoritableLineMixin:OnLoad()
	AuctionHouseItemListLineMixin.OnLoad(self);
...
AuctionHouseItemListMixin = CreateFromMixins(AuctionHouseBackgroundMixin);

function AuctionHouseItemListMixin:OnLoad()
	AuctionHouseBackgroundMixin.OnLoad(self);

	self.RefreshFrame:SetShown(not self:GetAttribute("hideRefreshFrame"));
	self.RefreshFrame:SetPoint("TOPRIGHT", self:GetAttribute("refreshFrameXOffset") or 0, self:GetAttribute("refreshFrameYOffset") or 0);

	self.NineSlice:SetPoint("BOTTOMRIGHT", -22, 0);

	self.ResultsText:SetPoint("TOP", self.ScrollFrame, "TOP", 0, -45);
	self.ResultsText:SetPoint("LEFT", 45, 0);
	self.ResultsText:SetPoint("RIGHT", -67, 0);
...
function AuctionHouseItemListMixin:Init()
	...
	self.LoadingSpinner.SpinnerAnim:Play();
	...
	for i, button in ipairs(self.ScrollFrame.buttons) do
		if self:GetAttribute("hideStripes") then
...
function AuctionHouseItemListMixin:UpdateRefreshFrame()
	...
	if self.state == ItemListState.NoSearch then
		self.RefreshFrame:Deactivate();
	elseif self.state == ItemListState.NoResults then
		self.RefreshFrame:SetQuantity(0);
	elseif self.state == ItemListState.ResultsPending then
		self.RefreshFrame:Deactivate();
