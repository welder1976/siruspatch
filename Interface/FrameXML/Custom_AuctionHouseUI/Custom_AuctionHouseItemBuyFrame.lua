...
local AUCTION_HOUSE_ITEM_BUY_FRAME_CUSTOM_EVENTS = {
...
function AuctionHouseItemBuyFrameMixin:OnShow()
	FrameUtil.RegisterFrameForCustomEvents(self, AUCTION_HOUSE_ITEM_BUY_FRAME_CUSTOM_EVENTS);
end

function AuctionHouseItemBuyFrameMixin:OnHide()
	FrameUtil.UnregisterFrameForCustomEvents(self, AUCTION_HOUSE_ITEM_BUY_FRAME_CUSTOM_EVENTS);
...
function AuctionHouseItemBuyFrameMixin:OnEvent(event, ...)
	...
	elseif event == "BIDS_UPDATED" or event == "AUCTION_CANCELED" then
		if self.ItemList.refreshResultsFunc then
			self.ItemList.refreshResultsFunc();
		end
	end
end
