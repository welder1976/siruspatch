...
function MultisellProgressFrameMixin:Start(itemTexture, total)
	...
	_G[self.ProgressBar:GetName().."Text"]:SetFormattedText(AUCTION_CREATING, 0, total);
	_G[self.ProgressBar:GetName().."Icon"]:SetTexture(itemTexture);
end

function MultisellProgressFrameMixin:Refresh(currentCount, totalCount)
	self.ProgressBar:SetValue(currentCount);
	_G[self.ProgressBar:GetName().."Text"]:SetFormattedText(AUCTION_CREATING, currentCount, totalCount);
