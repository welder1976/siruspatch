-- Нужно изменить при другой локализации клиента - CALENDAR_FIRST_WEEKDAY.
...
function CalendarEventDescriptionScrollFrame_OnLoad(self)
	...
	CalendarEventDescriptionScrollFrame_OnEvent(self, "ADDON_LOADED")
end
...
function CalendarEventInviteList_OnLoad(self)
	...
	CalendarEventInviteList_OnEvent(self, "ADDON_LOADED")
end
...
function CalendarViewEventFrame_Update()
	...
	if ( eventTex ) then
		-- set the event type
		CalendarViewEventTypeName:SetFormattedText(CALENDAR_VIEW_EVENTTYPE, safeselect(eventType, CalendarEventGetTypes()), GetDungeonNameWithDifficulty(eventTex.title, eventTex.difficultyName));
...
function CalendarCreateEventTexture_Update()
	...
	if ( eventTex ) then
		...
		CalendarCreateEventTextureName:SetText(GetDungeonNameWithDifficulty(eventTex.title, eventTex.difficultyName));
...
function CalendarEventPickerScrollFrame_OnLoad(self)
	...
	CalendarEventPickerScrollFrame_OnEvent(self, "ADDON_LOADED")
end
...
function CalendarTexturePickerScrollFrame_OnLoad(self)
	...
	CalendarTexturePickerScrollFrame_OnEvent(self, "ADDON_LOADED")
end
	...
	for i = 1, numButtons do
		...
		if ( eventTex ) then
			...
			if ( eventTex.textureIndex ) then
				...
				buttonTitle:SetText(GetDungeonNameWithDifficulty(eventTex.title, eventTex.difficultyName));
